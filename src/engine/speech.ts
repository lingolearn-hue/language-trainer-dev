...
import { audioHash, voiceIdFor } from "./audioHash";
import audioIndexData from "../data/audioIndex.json";

// Bundled at build time by scripts/tts-manifest.ts — the set of hashes
// that actually have a pre-generated public/audio/<hash>.mp3 file
// sitting in the repo right now. A plain array (not a Set) in the JSON
// itself since JSON has no Set type; wrapped in a real Set here once,
// at module load, for O(1) lookups instead of re-scanning an array on
// every speak() call.
const AUDIO_INDEX: Set<string> = new Set(audioIndexData as string[]);

// Plays a pre-generated clip if one exists for this exact (voiceId,
// lang, text) triple. Resolves true if it played to completion, false
// if there's no matching file OR playback failed for any reason (404,
// decode error, etc.) — false is the signal to fall through to
// WebSpeech below, not an error condition callers need to handle
// specially.
async function tryPreGeneratedAudio(text: string, lang: LangCode, preference: VoicePreference | undefined): Promise<boolean> {
  const voiceId = voiceIdFor(preference);
  const hash = await audioHash(voiceId, lang, text);
  if (!AUDIO_INDEX.has(hash)) return false;
  ...
  return new Promise<boolean>((resolve) => {
    ...
    audio.play().catch(() => finish(false)); // autoplay-policy rejection, decode failure, etc.
  });
}
...
export async function speak(
  ...
  setSpeaking(true);
  // Whole-text lookup, not per-segment: a pre-generated clip covers the
  // exact string passed in here, generated by a real TTS engine that
  // doesn't need WebSpeech's own segment-splitting workarounds (mixed
  // scripts needing different voices mid-utterance, ellipsis needing an
  // inserted pause) — those exist purely to route around WebSpeech's
  // own limitations, so when a pre-generated clip is available, this
  // skips straight past that whole subdivision and plays the one file.
  // Independent of the "speechSynthesis" in window guard below on
  // purpose: pre-generated playback uses a plain <audio> element, not
  // SpeechSynthesis, so it works even in a context that lacks
  // speechSynthesis entirely.
  if (await tryPreGeneratedAudio(text, lang, preference)) {
    setSpeaking(false);
    return;
  }
